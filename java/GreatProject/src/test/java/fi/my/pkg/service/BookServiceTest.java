...
public class BookServiceTest {
	...
	private void createBooksFromIsbnList() throws IOException, PdfFileNotFoundException {
		...
		for (String isbn : TestDataUtil.loadTestIsbnList()) {
			service.addBookToArchive(new PdfBook(id, isbn, "title" + id, Fixture.PATH + "test.pdf"));
			id++;
		}
	}
	...
	private void addTestAudioBooksWithTestIsbnList() throws IOException {
		int id = 1;
		for (String isbn : TestDataUtil.loadTestIsbnList()) {
			service.addBookToArchive(new AudioBook(isbn, "title" + id, Fixture.PATH + "test.pdf"));
			id++;
		}
	}
	...
	void testImport() throws Exception {
		service = new BookServiceImpl(mockStorage);
		service.importBooks(Fixture.IMPORT_PATH);
